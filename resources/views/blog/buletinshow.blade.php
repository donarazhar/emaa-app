...
<body class="bg-gray-100">
    ...
    <header class="bg-white text-white p-4">
        <div class="flex justify-between items-center">
            <a href="{{ route('buletin') }}" class="text-3xl font-bold">
    ...
    <section id="main" class="relative">
        ...
        <div class="w-full h-64 md:h-96">
            <img src="{{ $buletin->thumbnail ? asset('storage/' . $buletin->thumbnail) : 'https://placehold.co/600x400' }}"
                alt="{{ $buletin->judul }}" class="w-full h-full object-cover">
        </div>
    </section>
    ...
    <section id="content" class="p-4 mt-4 pb-20 overflow-auto">
        <div class="max-w-6xl mx-auto bg-white p-4 md:p-8 shadow-md">
            <h1 class="text-xl md:text-2xl font-bold mb-2 md:mb-4">{{ $buletin->judul }}</h1>
            <p class="text-sm md:text-base text-gray-600 mb-2 md:mb-4">
                {{ \Carbon\Carbon::parse($buletin->tanggal_jam)->format('d F Y H:i') }}</p>
